...
public:
	...
	void setSize(int newSize) { int treeSize = 2 * pow(2, (ceil(log(newSize) / log(2)))+1); m_tree.resize(treeSize); m_lazy.resize(treeSize); }
	...
	void init(const vector<int>& values) {
		...
		initialize(1, 0, inputSize - 1, values);
	}	
	...
	void update(int nodeid, int ss, int se, int rs, int re, int deltaVal) {
		...
		//Transfer lazy updates to main tree and to child nodes of lazy tree only if the current node is not the child node.
