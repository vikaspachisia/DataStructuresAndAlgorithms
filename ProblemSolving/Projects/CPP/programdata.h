...
#pragma region programdata

/*
A base class that is responsible to manage the data for the program.
Basically programdata is responsible for reading from input test files, 
writing to ouput results file and sanitizing corruput data.
...
protected:
	...
	int line_number = 0;
	string m_filepath{};
	string m_filepathroot{ "..\\DataFiles\\" };
	fstream m_datastream { };
	string m_name{};
public:
	programdata(string name) :m_name(name){}
...
#pragma region programdata_reader

/*
The reader class can be used to read input for the program from a file or from standard console.
When redirection is enabled, the standard input such as 'stdin' or 'std::cin' will also be redirected from the file.
*/
class programdata_reader : public programdata {
private:
	string lastLine{};
public:
...
#pragma region programdata_writer
...
*/
class programdata_writer : public programdata{
public:
